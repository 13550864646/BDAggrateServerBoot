...
public class No8OneWayParser extends UnitASportComponent {
    ...
    public PackageData generateRealPackageData(IoBuffer buffer) {
        ...
        String firmware_version = DeviceIDResolver.getFirrnwareVersion(buffer.array(), 18);
        String prefix = DeviceIDResolver.getDeviceIDPrefix(buffer.array(), 45);
        String deviceID = DeviceIDResolver.getDeviceIDFrornBytes(buffer.array(), 50);
        String patientID = DeviceIDResolver.searchPatientidByDeviceid(deviceID);
        String company = DeviceIDResolver.searchCornpanyByDeviceid(deviceID);
